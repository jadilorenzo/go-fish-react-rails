...
class GoFish
  ...
  def play_bot_rounds
    while turn_player.class == BotPlayer
      return if over?
      play_bot_round
    end
  end
  ...
  def return_opponent_ids
    players.reject {|player| player == turn_player}.map {|player| player.user_id}
  end
  ...
  def handle_emptiness
    until over? || !turn_player.hand.empty?
      check_emptiness
    end
  end
  ...
  def winner
    ...
    players.reject {|people| people.books.count != book_counts.max }
  end
  ...
  def self.from_json(json)
    ...
    players_from_json = json[:players].map {|player| player[:user_id] < 0 ? BotPlayer.from_json(player) : Player.from_json(player)}
    deck_from_json = Deck.from_json(json[:deck])
    self.new(players_from_json, deck_from_json, json[:started_status], json[:round_count], json[:round_results])
  end
